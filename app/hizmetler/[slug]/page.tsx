...
export default async function ServicePage({ params }: Props) {
  ...
  return <><JsonLd data={schema}/><SiteHeader/><main className="inner-page">
    <section id="hizmet-baslangici" className="service-hero technical-paper"><div><p className="kicker">{service.eyebrow}</p><h1 className="font-display">{service.name}<br/><em>uygulamaları.</em></h1></div><div><p>{service.shortAnswer}</p><Link className="btn-primary" href={screenCatalog ? "#teklif" : `/teklif-al?hizmet=${service.slug}`}>{screenCatalog ? "Ölçüyle fiyat oluştur" : `${service.name} keşfi iste`} <ArrowUpRight size={17}/></Link></div></section>
    ...
    {screenCatalog && <section className="screen-system-section"><div className="screen-system-head"><div><p className="kicker">Üç net çözüm</p><h2 className="font-display">Alanınızı seçin,<br/><em>ölçünüzle teklif alın.</em></h2></div><p>Kafa karıştıran model listeleri yerine yalnız üç kullanım çözümü sunuyoruz. Uygunluğu ekibimiz ölçü üzerinden kontrol eder.</p></div><div className="screen-system-grid">{screenCatalog.products.filter(product=>product.active&&["sabit-citcitli","surme","duble"].includes(product.slug)).map((product,index)=>{const copy=SCREEN_SYSTEM_COPY[product.slug];const title=product.slug==="duble"?"Duble sürgülü":copy?.title||product.name;return <a href="#teklif" key={product.slug}><figure><Image src={PRODUCT_IMAGES[product.slug]?.hero || "/services/sineklik-v2.webp"} alt={`${title} sineklik çözümü`} fill sizes="(max-width:700px) 100vw, (max-width:1100px) 50vw, 33vw"/></figure><span>0{index+1} · Çözüm</span><h3 className="font-display">{title}</h3><p>{copy?.description||product.description}</p><small>Uygun alan: <b>{copy?.bestFor||"Kapı ve pencere açıklıkları"}</b></small><strong>Ölçünü gir, teklifini gör <ArrowUpRight size={14}/></strong></a>})}</div></section>}
