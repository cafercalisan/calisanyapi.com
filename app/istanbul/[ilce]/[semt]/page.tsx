...
import { getDistrict, site } from "@/lib/site";
...
export default async function NeighborhoodPage({ params }: Props) {
  ...
  return <><ContentTracker type="neighborhood" slug={semt} district={ilce} neighborhood={semt}/><JsonLd data={{"@context":"https://schema.org","@graph":[{"@type":"Service",name:`${item.name} yapı uygulamaları`,areaServed:{"@type":"Place",name:`${item.name}, ${district.name}`},provider:{"@id":`${site.url}/#organization`},url},{"@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"Ana Sayfa",item:site.url},{"@type":"ListItem",position:2,name:"İstanbul",item:`${site.url}/istanbul`},{"@type":"ListItem",position:3,name:district.name,item:`${site.url}/istanbul/${ilce}`},{"@type":"ListItem",position:4,name:item.name,item:url}]}]}}/><SiteHeader/><main className="inner-page"><section className="geo-hero technical-paper"><div><p className="kicker">İstanbul · {district.name} · {item.name}</p><h1 className="font-display">{item.name}’nda<br/><em>ölçüye özel uygulama.</em></h1></div><div><p>{item.summary}</p><Link className="btn-primary" href={`/teklif-al?ilce=${ilce}&from=${encodeURIComponent(path)}`}>{item.name} için keşif iste <ArrowUpRight size={16}/></Link></div></section><LocalServiceShowcase placeName={item.name}/><section className="geo-proof"><div><span>01</span><h2 className="font-display">Yerinde ölçü</h2><p>Uygulama alanını, bağlantı yüzeylerini ve kullanım beklentinizi birlikte değerlendiririz.</p></div><div><span>02</span><h2 className="font-display">Açık kapsam</h2><p>Malzeme, sistem, söküm ve montaj detaylarını işe başlamadan önce netleştiririz.</p></div><div><span>03</span><h2 className="font-display">Planlı uygulama</h2><p>Üretim ve montaj adımlarını alanın koşullarına göre programlarız.</p></div></section></main><SiteFooter/></>;
}
