...
async function notify(customerEmail: string | undefined, name: string, reference: string, total: number) {
  ...
  await Promise.allSettled(recipients.map((to) => fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" }, body: JSON.stringify({ from: process.env.FROM_EMAIL || "Uma Yapı <noreply@umayapi.com>", to: [to], subject: `${reference} numaralı teklif alındı`, html: `<div style="font-family:sans-serif;max-width:560px;margin:auto;padding:32px"><h1>Uma Yapı</h1><p>Merhaba ${escapeHtml(name)},</p><p>${reference} numaralı teklifiniz kaydedildi. Hesaplanan toplam: <strong>${new Intl.NumberFormat("tr-TR", { style: "currency", currency: "TRY" }).format(total)}</strong>.</p><p>Ölçü ve ürün uygunluğu doğrulandıktan sonra ekibimiz sizinle iletişime geçecektir.</p></div>` }) })));
}
